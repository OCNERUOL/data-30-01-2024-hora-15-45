...
function askBot() {
    $("#send_button").click(function () {
        ...
        if (user_bot_input_text != "") {
            ...
            $.ajax({
                type: 'POST',
                url: "/bot-response",
                data: JSON.tringify(chat_input_data),
                dataType: "json",
                contentType: 'application/json',
                    success: function (result) {
                        $("chat_messages").append(
                            '<div class="bot__messages">'
                            + result.bot.response + ' </div>')

                        $(".chatbox__messages__container").animate({
                            scrollTop: $(
                                '.chatbox__messages__container'
                                )[0].scrollHeight}, 1000);
                            },
                            error: function (result) {
                                alert(result.responseJSON.message)
                            }           });         }    })
    $('#bot_input_text').keypress(function(){
        //se a tecla Enter (codigo de tecla 13) for pressionada
        if(e.which == 13){
            $('#send_button').click(); //dispara o evento de clicar o enter
        }
    })                            
    ...
    $('#').keypress(function(e){
        ...
        if(){         
            $('#').click(); //Acionar Evento de Clique do Botão Enviar
        }
    });
}
